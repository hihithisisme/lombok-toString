...
func TestParseLombokStringAsJSON(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.subtestName, func(t *testing.T) {
			...
			obj := parser.Process(parser.SegmentByBrackets(tc.lString))
...
//	}
//
//	obj := lombokString.Process(lombokString.SegmentByBrackets(tc.lString))
//	actual := lombokString.ParseAsJSON(obj, lombokString.InterfaceArgs{})
//
//	for _, substring := range tc.shouldContains {
//		assert.Contains(t, actual, substring)
//	}
